...
def main():
    ...
    for i in range(len(style_images)):
        ...
        scale = style_scale * target_shape[1] / style_images[i].shape[1]  
        new_shape = (int(math.floor(scale*style_images[i].shape[0])), int(math.floor(scale*style_images[i].shape[1])))  
        style_images[i] = skimage.transform.resize(style_images[i], new_shape)
